...
| `task_manager` | task, todo, to-do, backlog, mark done, blocked, in progress | none | none |
...
| `session_tool` | rename session, list sessions, past session, what did we talk about, previous sessions, summarize our chats | none | none |
| `code_writer` | create a tool, write tool code, scaffold | none | none |
| `code_runner` | run python, execute code, test snippet, verify output | none | none |
